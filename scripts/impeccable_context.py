...
def render_design() -> str:
    ...
    return "\n".join(
        [
            ...
            direction,
            "",
            "Approved design-system v1.0 is Shipboard Terminal. Do not treat",
            "v0.1 Deep-Space styling as the target look for the frontend rebuild.",
            "",
            "Semantic tokens (dark canonical): canvas `#07141B`, phosphor teal",
            "`#3CC0BF` (`brand-primary` / live), signal amber `#E2A33C` (attention),",
            "text `#E6EEF2` / `#F8FCFE`, hairline `rgba(110,154,156,0.52)`.",
            "Typography: Michroma placards, Sometype Mono body. Geometry: zero",
            "radius with 10px notches. Light theme maps the same roles.",
            "",
