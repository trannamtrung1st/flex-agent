...
Follow approved specs, existing architecture, `AGENTS.md`, and the equivalent Cursor foundation rule when present.

## Responsibilities

- Implement observable contracts from approved requirements and acceptance criteria.
- Follow approved architecture decisions; use `architect` when a change introduces or revises cross-cutting boundaries, quality attributes, technology choices, or deployment topology.
