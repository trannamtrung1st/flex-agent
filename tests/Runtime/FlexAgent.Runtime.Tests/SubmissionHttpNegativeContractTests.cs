...
public sealed class SubmissionHttpNegativeContractTests
{
    ...
    [Fact]
    public async Task Intake_query_without_a_session_is_unauthorized_and_not_cached()
    {
        await using var factory = CreateFactory();
        var client = factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        using var response = await client.GetAsync(
            $"/v2/assessment/my-work/{Guid.CreateVersion7()}/submission/intake/{Guid.CreateVersion7()}",
            TestContext.Current.CancellationToken);
        var body = await response.Content.ReadAsStringAsync(TestContext.Current.CancellationToken);
    ...
    private sealed class ThrowingSubmissionQueryService : ISubmissionQueryService
    {
        ...
        public Task<QueryResult<IntakeMutationOutcome>> GetIntakeAsync(
            EnrollmentActorContext actor,
            Guid enrollmentId,
            Guid intakeId,
            CancellationToken cancellationToken = default) =>
