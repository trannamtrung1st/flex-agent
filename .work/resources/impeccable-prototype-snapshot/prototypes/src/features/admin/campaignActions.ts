...
export function createCampaignActions(handlers: {
  ...
  return [
    ...
    },
    {
      ...
      placement: "primary",
      eligibility: (records) => {
        const drafts = records.filter((campaign) => !campaign.frozen);
        if (drafts.length) {
          return {
            allowed: false,
            reason: drafts.length === records.length
              ? "Configuration snapshot is available after activation"
              : `${pad(drafts.length)} draft ${drafts.length === 1 ? "campaign is" : "campaigns are"} not yet activated`,
          };
        }
        return { allowed: true };
      },
    ...
    },
    {
      ...
      eligibility: (records) => {
        ...
        if (frozen.length) {
          return {
            allowed: false,
            reason: frozen.length === records.length
              ? "Activated campaigns are retained in this prototype"
              : `${pad(frozen.length)} activated ${frozen.length === 1 ? "campaign is" : "campaigns are"} retained in this prototype`,
          };
        }
        return { allowed: true };
      },
