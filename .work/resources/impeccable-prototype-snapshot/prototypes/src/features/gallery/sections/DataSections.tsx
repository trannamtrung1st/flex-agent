...
import { EMPTY_SELECTION, isSelected, matchingQueryKey, resolveSelectedIds, toggleRow, type TableSelection } from "../../../components/tableSelection";
import {
  ActivationMark,
  ChevronGlyph,
  CommandMenu,
              ...
              {visible.map((row) => (
                <Fragment key={row.id}>
                  <tr className={`datatable-row${isSelected(selection, row.id) ? " is-selected" : ""}${expanded === row.id ? " is-expanded" : ""}`}>
                    <td className="cell-select"><SelectMark checked={isSelected(selection, row.id)} label={`Select ${row.id}`} onChange={(checked) => setSelection((current) => toggleRow(current, row.id, checked))} /></td>
