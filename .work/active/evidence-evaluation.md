...
status: in-progress
created: 2026-09-07
updated: 2026-09-13T00:35:00+07:00
phase6_slice2_wider_ac_eval24: pending-push
...
- [>] Red: prompt-injection and confused-deputy suites across Submission text,
  ...
  **Wider AC-EVAL-24 matrix (`[>]`):** `EvidenceSourceWiderMatrixPromptInjectionTests`
  ...
  `AC-EVAL-24`. Focused: Evaluation unit **354**; `verify-dotnet.sh` green
  (local). **Pending push, hosted CI, and external review.**
  **Slice 2 remainder:** wider AC-EVAL-24 matrix external review/CI; credential
...
- [>] Independently validate every model response for schema, exact criterion
  ...
  **Phase 6 slice 2 not closed** — wider AC-EVAL-24 matrix tests landed locally
  (`EvidenceSourceWiderMatrixPromptInjectionTests` 11); pending push/review/CI.
  Worker disabled.
...
- **Phase 6 slice 2 in progress.** Store/authority-binding increment **approved**
  ...
  **0/0/0/0**; Implementation CI `34706092150`. Focused:
  ...
  `68b35ee7`; external re-review **0/0/0/0**; Implementation CI `34706092150`).
  **Wider AC-EVAL-24 matrix:** `EvidenceSourceWiderMatrixPromptInjectionTests`
  11 landed locally; pending push/review/CI. **Slice 2 not closed.** Worker
  disabled. **Do
...
| Phase 6 slice 2 wider AC-EVAL-24 matrix (filenames, metadata, agent messages, knowledge, deterministic output) | pending | `EvidenceSourceWiderMatrixPromptInjectionTests` 11: text_attachment; filename authority invariant; agent fragment assembly; manifest.fact stripping; knowledge-shaped configuration; deterministic.fact locator/completion; metadata projector injected source_id negative. Model-output boundary covered by approved model-boundary suite. No production code change. Focused: Evaluation unit **354**; `verify-dotnet.sh` green (local). **Pending push, hosted CI, and external review.** Worker disabled |
