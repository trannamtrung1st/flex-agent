...
| `REQ-SESS-64`–`67`, `70`, `78`–`85`, `AC-SESS-42`–`47`, `SESS-DEC-17`–`19`, `23`, `29`–`35` | Successor Decision envelope + validation/effect; v1 dual-read | Decision commit; independent output vs action validation; separate effect TX | Reauth at effect | Decision id + output id + effect idempotency | `session.agent.work.v1` resolved state | `respond`+one message / `no_action`+zero outputs; timer action independent | Empty outputs without disposition; voice/extra message/audience/id; schema-invalid → execution outcome; policy reject → no effect |
