...
- [x] Remediate `1c16281` P2: after-accept refresh clears accepted local
  material even when a later intake is already active.
- [ ] Independent review of the `1c16281` P2 remediations.
...
The slice remains **in progress**. Independent review of `1c16281` found one
P2: **Refresh assignment** after a successful I1 accept could install a later
active I2 without clearing I1's local text/files, so those materials could be
submitted again after I2 ended.

Remediation: `after-accept` cleanup of local material runs before applying a
newer active intake's status. Refresh with I2 receiving shows receiving, empty
direct text, and Submit disabled.

Red: after-accept refresh with I2 left the accepted I1 text in the textarea.
Green: My work tests **18 passed**; web typecheck passed. Playwright was not
re-run; this hop is a mocked two-tab race.
...
Next: independent review of the `1c16281` P2 remediations. Do not mark the
...
- **Review of `1c16281` (P2, remediating):** After-accept refresh clears
  accepted local text/files before applying a later active intake's status.
