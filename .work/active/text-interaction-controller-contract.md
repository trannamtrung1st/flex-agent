...
- `AGENTS.md` and `docs/README.md` — authority by concern, product invariants,
  ...
  tracked-plan rules
- Git `3f85078:.work/resources/text-interaction-controller-proposal.md` —
  non-authoritative source proposal to evaluate, revise, approve, or reject
  (file retired from `.work/resources/`; recover from Git)
- Structured Agent runtime sync — prerequisite implementation is complete;
  recover the retired task from Git if evidence reconstruction is needed
- Shipboard production UX reset — complete; current Text Session UI owner is
  `docs/ui-ux/flows/text-session.md`
...
- `docs/ui-ux/flows/text-session.md` — current approved Text Session journey
  owner, plus the approved design-system status, implementation guide, and
  applicable interaction modules
- `docs/architecture/session-runtime-contract.md` — durable streaming,
  Invocation/Decision, one-lane timer replacement, output envelope, ordering,
  cutoff, and future Interaction Controller seams
...
- The structured-agent-runtime-sync dependency is complete. This task remains
  `planned` because explicit Product Lead prioritization is still required. It
  is not activated by this restore.
...
- Current Text Session UI authority is `docs/ui-ux/flows/text-session.md`. This
  task must not author or implement Session UI behavior until Product Lead
  activation.
...
- Compare amending `docs/architecture/session-runtime-contract.md` against
  another current architecture owner if controller seams do not fit that
  document. Do not create a decision-record file.
- Publish only explicitly approved decisions in the appropriate current
  authoritative artifacts with stable requirement/acceptance IDs, traceability,
  and implementation-status honesty. Git preserves prior text.
...
- Changes to the completed structured-agent-runtime-sync acceptance target
  while this task remains planned.
...
- [x] Confirm current approved Text Session authority
  (`docs/ui-ux/flows/text-session.md`) before any UI/UX authoring.
...
- [ ] Update the current architecture owner for controller ownership, logical
...
Planned and awaiting explicit Product Lead prioritization. Restored on
2026-09-01 after Phase 5 deleted this file without owner reconfirmation. It is
not activated.

The structured runtime dependency and its successor host slices are completed,
including the structured Invocation/Decision, response-streaming, production
HTTP SSE, Session binding, and one-lane timer foundation. Interaction
Controller triggers remain disabled and deferred by the approved MVP scope.

Current Text Session UI authority is `docs/ui-ux/flows/text-session.md`. If the
Product Lead activates this contract task, the first substantive step is an
evidence-based seam inventory against the completed runtime; do not begin by
copying the proposal into authoritative documents or by treating the restored
plan as product approval.
...
| --- | --- | --- |
| Dependency status and completion evidence | complete | Structured-agent-runtime-sync and Worker, production HTTP SSE, subject-binding, and timer-activation successors completed as of 2026-08-18; recover retired tasks from Git |
| Proposal remains non-authoritative and does not enable behavior | complete | Status and dependency statements in this task; proposal blob at Git `3f85078:.work/resources/text-interaction-controller-proposal.md` |
| Proposal cross-concern consistency review | complete | Checked against `REQ-SESS-10`, `REQ-SESS-61`–`REQ-SESS-70`, `REQ-SESS-84`, `SESS-DEC-14`–`SESS-DEC-23`, and Text Session/design-system interaction boundaries; corrections applied 2026-08-15 |
| Production UI reset dependency | complete | Current approved Text Session owner is `docs/ui-ux/flows/text-session.md` |
...
- This task stays `planned` until Product Lead activation. Current Text Session
  UI owner is `docs/ui-ux/flows/text-session.md`.
...
- [ ] Current architecture owner update and security/privacy review are approved
