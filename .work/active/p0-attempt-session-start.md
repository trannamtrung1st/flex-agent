...
Review of `5a05833` (2026-09-02): Session runtime now stores the resolved
configuration digest independently from the frozen policy digest. Attempt
bindings persist a version-level content digest and round-trip it. Bind and
ordinary start failures happen before Session writes so failed start operations
remain inspectable; unexpected post-write failures still abort then persist the
failed operation. Definitive browser Start failures clear the idempotency key.
Migration `0065` and `session_*` schema-test coverage include acknowledgment,
resolved-configuration, and initial-manifest tables.
...
| Focused Submissions domain/application tests | passed (partial) | 2026-09-02: AttemptStartCoordinatorTests 9 passed; AttemptDomainTests 20 passed |
...
| PostgreSQL migration/integration/fault tests | passed (partial) | 2026-09-02: abort + gated digest alignment 2 passed; binding source distinct digests 6 passed; session prefix schema 1 passed; 0065–0066 on upgrade lists |
...
| Web unit/component tests | passed (partial) | 2026-09-02: ProductionMyWorkDetailPage 7 passed including new start-key after definitive failure |
