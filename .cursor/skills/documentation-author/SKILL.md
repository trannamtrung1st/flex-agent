...
Do not average conflicting specialist views. Surface the conflict, decision owner, options, impact, required approval, and an **interim default** with brief rationale.
...
4. Separate confirmed facts, normative requirements, decisions, proposals, assumptions, examples, open questions, and deferred scope. Every open question must include an **interim default** with brief rationale; that interim default is working guidance only and does not govern behavior until approved.
...
- Make requirements observable and acceptance criteria testable; replace vague claims with measurable thresholds or an open question that includes an **interim default** and brief rationale.
...
Report documents created or changed, their status, specialist perspectives applied, decisions captured, open questions (each with **interim default** and rationale), conflicts, traceability gaps, and downstream artifacts that may now be stale.
