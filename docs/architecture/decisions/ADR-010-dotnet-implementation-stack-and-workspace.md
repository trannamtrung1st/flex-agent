...
| **Approval reference** | Explicit owner approval in the 2026-08-07 implementation-stack and follow-up schema/canonicalization reviews; Product Lead approval on 2026-08-19 of the separate OpenRouter synthetic-development adapter/profile boundary; 2026-08-20 owner delegation approved the suitable technical realization of the OIDC/application-session business decisions; Product Lead approval on 2026-08-20 replaced the vendor-specific Direct OpenAI target with one OpenAI-compatible endpoint contract; on 2026-08-21 the repository owner explicitly approved the amendment acting for the required Architecture, Backend, Frontend, Security/Privacy, and Operations approvers, with Product concurrence, clarifying IdentityAccess/Keycloak ownership and the reproducible authenticated local-browser profile |
...
| `STACK-DEC-11` | Implement one application-owned OpenAI-compatible adapter behind the provider-neutral `ModelProvider` port. Its stable identity is the vendor-neutral adapter kind `openai_compatible` and contract `sessions.openai_compatible.v1`; the supported wire surface is the explicitly tested Chat Completions subset, not an assertion of complete OpenAI API compatibility. Require a versioned adapter configuration whose digest is included in the installed deployment-profile identity; it owns the exact API base path and destination-policy identity while the profile retains the approved HTTPS origin. The current official OpenAI .NET SDK may remain an internal transport dependency only while it correctly supports qualified custom endpoints; it does not define the adapter contract. OpenRouter remains a separate synthetic-development-only adapter because of its additional routing and policy semantics, and vLLM remains an optional self-hosted target rather than a model default. | The adapter can serve approved operator-installed Organization-hosted, self-hosted, managed, or OpenAI-hosted endpoints without assigning any provider preferred status or enabling Organization self-service endpoint entry. Provider-specific headers, authentication differences, feature differences, usage, cancellation, retry, normalized failure, and provenance stay inside reviewed adapters and contract tests. Protocol compatibility is exact-profile evidence, not an origin or SDK assumption, and no adapter chooses a fallback silently. |
...
| `STACK-DEC-18` | Implement OpenRouter as a distinct Sessions-owned adapter contract behind the existing provider-neutral execution port. Its approved development surface is OpenAI-compatible Chat Completions at the fixed `/api/v1` base path; OpenRouter routing fields, opt-in router metadata, response-cache denial, returned identity, and failure semantics remain adapter concerns. Do not reuse the generic `openai_compatible` adapter kind or treat an origin-only profile substitution as compatibility evidence. The 2026-08-20 amendment requires an explicit synthetic-data-policy acceptance gate for the retention/training-enabled development profile. | A separate adapter makes OpenRouter path handling, free-router discovery, concrete `:free` model and provider pinning, synthetic-only data-policy acceptance, no-fallback behavior, and provenance independently testable without weakening the generic OpenAI-compatible contract or durable domain contracts. Production/Staging and real data remain prohibited. |
| `STACK-DEC-19` | Resolve human login only through an exact, pre-provisioned `(issuer, subject)` binding to an enabled actor, then derive exactly one eligible Organization from application-owned records and bind it immutably to the application session. Unknown, disabled, rebound, zero-Organization, or multi-Organization results fail closed; the callback accepts no client-selected Organization or provider role/group authority. | Authentication must not provision or widen authorization. One server-derived Organization preserves ADR-002 isolation and the approved MVP interaction boundary. |
| `STACK-DEC-20` | Persist the ASP.NET Core Data Protection key ring in PostgreSQL for all API instances and protect keys at rest with an operator-mounted encryption certificate or equivalent approved secret-facility key. Persist bounded, expiring, single-use OIDC transaction records for state/nonce/correlation and PKCE continuity; store secrets encrypted or as purpose-appropriate digests, consume atomically, and do not require callback affinity. | OIDC callback verification must survive instance changes without an instance-local key ring or memory store becoming authentication authority. Durable single-use consumption supplies replay and cleanup evidence. |
| `STACK-DEC-21` | Retain no provider access or refresh token after validated callback completion. Store only bounded validated authentication-strength facts, a keyed lookup digest of any provider session identifier required for logout propagation, and an encrypted provider ID token ciphertext bound to the live application session solely for RP-initiated logout `id_token_hint`. Clear that ciphertext when the session terminates; never expose it through session APIs, logs, or client responses except as the one-time `id_token_hint` on logout. Administrator and Reviewer access accepts only an operator-configured allowlist of exact `acr`/`amr` evidence qualified by the pinned Keycloak profile; absent, malformed, stale, or unrecognized evidence denies the protected action. | This minimizes credentials and claims while preserving application-owned MFA enforcement, revocation correlation, and frictionless unified-platform SSO logout through Keycloak 26 OIDC RP-initiated logout. |
| `STACK-DEC-22` | Revoke the Flex Agent application session atomically and clear its cookie before attempting a bounded provider logout. A provider logout failure cannot restore or prolong local authority. The Keycloak reference profile must implement a trusted lifecycle-propagation adapter: validated back-channel logout for provider-session termination and a restricted, authenticated Keycloak event-listener bridge for account disablement. The bridge maps only exact configured issuer/subject or provider-session references and revokes matching local sessions so protected requests and SSE lose authority within 60 seconds. | Keycloak availability cannot sit on every request, yet the approved disablement/forced-logout bound needs positive trusted propagation. A narrow authenticated bridge is auditable and testable without retaining bearer-capable provider access or refresh tokens (`STACK-DEC-21` governs the bounded logout-only ID-token ciphertext) or granting provider roles application authority. |
| `STACK-DEC-23` | IdentityAccess owns a deployment-scoped, append-only, minimized authentication-security event stream for login and callback facts that occur before Organization resolution. After an Organization is resolved, ADR-003 continues to own Organization-scoped authorization audit. Pre-context events never use a fabricated Organization and follow the approved 730-day authentication/security metadata lifecycle. | The existing Organization-required audit schema cannot truthfully represent pre-context authentication. Separate ownership preserves provenance, minimization, and tenant semantics. |
| `STACK-DEC-24` | Use PostgreSQL time for application-session creation, activity, expiry, rotation, and revocation. A successful authenticated browser request, command, or client-initiated SSE connect/reconnect may atomically advance `last_seen_at`; server heartbeats, server polling, provider checks, and background revalidation do not. Implementations may coalesce writes only if the persisted value and conservative expiry calculation can never extend the approved 30-minute inactivity bound. | Database time gives multi-instance ordering, and client-originated activity reflects actual use without background traffic keeping abandoned sessions alive. Conservative coalescing permits measured optimization without changing behavior. |
| `STACK-DEC-25` | Expiry, logout, revocation, and rotation make the old lookup digest unusable immediately and preserve append-only rotation/termination lineage. Clear or irreversibly minimize bearer-capable material at terminal transition; retain only bounded session/security metadata under the approved 730-day authentication-audit lifecycle, then dispose of it dependency-safely. | Terminal credentials have no execution purpose, while audit-relevant history must remain inspectable and must not be silently overwritten. |
| `STACK-DEC-26` | Keep application user administration within the IdentityAccess business boundary. Keycloak owns credentials, MFA enrollment and evidence, authentication, and upstream account lifecycle. IdentityAccess owns internal actors, exact `(issuer, subject)` bindings, enabled state, Organization membership/context, capability grants, service delegations, application sessions, lifecycle propagation, and authentication-security events. Resource-owning modules retain authority for their resource relationships and workflow state and expose only versioned trusted inputs through owned ports to the authorization kernel. Do not create a parallel `UserManagement` module or treat Keycloak roles/groups as Flex Agent authorization. A later invitation, actor-administration, membership, or account-provisioning journey must be specified as an IdentityAccess-owned capability and may use a provider-specific Keycloak administration adapter. | One owner prevents duplicate actor, membership, session, and authorization authority without moving Cohorts, Enrollments, reviewer assignments, Session ownership, or other resource relationships out of their owning modules. Keycloak remains replaceable, provider credentials remain outside application records, and future administration can evolve without coupling domain permissions to provider roles. |
| `STACK-DEC-27` | Provide a reproducible Development/Testing authenticated-browser profile through one documented project command. Its canonical browser-visible origin is `http://localhost:18080`: the NGINX gateway serves or proxies the SPA at `/`, routes `/auth`, `/v1/assessment`, and existing `/sessions` requests to the API, and exposes the pinned Keycloak realm at `/realms/flex-agent`. The exact OIDC redirect URI is `http://localhost:18080/auth/callback`, returning the browser to the same public origin. The profile composes Keycloak, PostgreSQL, migrations, API, SPA/gateway, synthetic seed/reset, and health/readiness checks on one network; the API and migration operation reach PostgreSQL at `postgres:5432`. It includes an MFA-qualified synthetic Administrator whose exact provider identity is pre-bound to one enabled internal actor, one Organization, the minimum application-owned capability grants, and any Assessment-owned relationship records required by the journey. It uses synthetic data only and remains distinct from Production certification. | Real browser verification must exercise OIDC, the opaque application session, server-owned authorization, PostgreSQL Assessment state, and the production API boundary without split-origin or callback ambiguity. A frontend-only Vite session or synthetic `/browser` adapter cannot provide that evidence, while a bounded disposable profile avoids requiring external credentials. |
...
| `GATE-STACK-PROVIDERS` | Model compatibility | Fake, generic OpenAI-compatible, synthetic OpenRouter, and exact runtime/profile contract tests cover streaming, structured outputs used by the MVP, timeouts, cancellation, retries, failure normalization, usage/provenance, immutable returned identity, and fail-closed no-fallback behavior. OpenAI-compatible evidence must identify the actual provider/operator, endpoint, model/version-or-fingerprint, capability profile, credential mode, and data policy; an OpenAI-hosted run receives no special qualification status. Organization-hosted private endpoints additionally require destination-policy, DNS/rebinding, TLS, egress, and cross-Organization negative evidence. Synthetic OpenRouter evidence must distinguish random-router discovery from a pinned concrete model/provider, attest selected routing from metadata, reject response-cache hits, and cannot satisfy a real-data provider-profile gate |
