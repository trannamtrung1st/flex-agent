...
Sessions that bind the terminal cutoff sequence as evaluation-boundary provenance use `manifest-jcs-sha256-v2`, which extends that document with `cutoff_sequence`. `manifest-jcs-sha256-v1` remains the original published field set and must not be reinterpreted as if it had covered cutoff. Historical `session_terminal_records` created before seal columns existed keep NULL procedure and seal fields as the legacy-unsealed representation; those rows are append-only and must not be rewritten with a fabricated seal. They are not evaluation-handoff eligible. An eligible Evaluation handoff must reference the sealed terminal record and carry the same lifecycle state, cutoff sequence, procedure, and digest; it cannot claim eligibility independently of that seal. The handoff's configuration identifier, configuration digest, and manifest identifier must match the Session's immutable binding.
