...
Approved — 2026-08-26; amended 2026-08-26 to require protected-subtree remount on identity replacement, fresh permission observation before dependent reads, exact no-refetch Activities invalidation, an authorization-context epoch on every successful trusted-context replacement, to keep synthetic command follow-up refresh distinct from that replacement, to tear down protected synthetic UI on every ready-exit, to propagate synthetic HTTP 401 authentication loss from command and resource fetches, and to ignore stale 401s from a previous authorization-context epoch.
...
| `FE-DEC-4` | Treat every Query/mutation cache entry as protected until an explicit reviewed public/protected split exists. Every transition out of API `ready`, plus every successful trusted-context replacement, must cancel in-flight work, clear the complete QueryClient (including mutation variables and results), bump an authorization-context epoch, bump generation-based stale-response protection, and remount the protected React subtree. Same actor and Organization with a narrowed relationship, navigation, or permission set is still a new authorization context. Query-cache clearing alone does not discard RHF, refs, or other local protected UI state. | `WI-FE-03`, `WI-FE-14` |
...
| `FE-DEC-11` | Realtime Session projection, SSE/`EventSource` lifecycle, pending command identity, reconciliation, reconnect, and Session isolation remain outside this Query/form migration. Ordinary synthetic command follow-up actor/navigation refresh is not a trusted-context replacement and must not remount Session UI. Leaving API `ready` for unauthenticated, denied, or error still must purge protected state, clear actor/navigation, advance the authorization-context epoch, and stop rendering protected routes. Synthetic HTTP 401 from command, reconcile, or resource fetch is authentication loss and must use that same ready-exit, except a 401 whose request started under a previous epoch must not sign out the current context. Command HTTP 403/409 domain outcomes are not workspace access loss. Same-actor capability or `actor_stage` narrowing is treated as replacement until a server-issued authorization/session version exists. Session realtime contracts may be refactored only in a dedicated task that preserves the reviewed contracts. | `WI-FE-13` |
| `FE-DEC-12` | Shell authentication, CSRF, logout, generation-based stale-response protection, actor/navigation bootstrap, and protected-state clearing remain in the existing API providers. Query cache clearing is a dependent lifecycle, not a replacement identity layer. Successful trusted-context replacement remounts the protected subtree (`ProtectedAuthSubtree`) keyed by actor, Organization, and authorization-context epoch without remounting the API provider. | `WI-FE-03`, `WI-FE-06` |
