...
claims the exact addressed schedule revision, reauthorizes and revalidates
lifecycle, policy, budget, and cutoff, then commits one trusted timer trigger
and one Invocation admission. A retry must name that same revision: if it
