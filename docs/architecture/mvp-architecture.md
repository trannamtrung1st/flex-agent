...
| Enrollment and Submission | Assignment availability, deadline/timezone, entitlement, accommodation summary, intake progress, quarantined/validating/rejected/accepted state, immutable version, capability compatibility, start eligibility, starting, active, exhausted, uncertain outcome, and recovery action |
...
| Before frontend implementation | Apply the approved [Activity/Campaign journey](../ui-ux/activity-campaign-journey.md) and [assessment Campaign setup interaction specification](../ui-ux/assessment-campaign-setup.md); author approved interaction specifications for Submission, Session, Evaluation/review, and Result/Release states. | The platform IA, P0 journey, and setup interaction specification are approved; four surface interaction specifications remain the UI/UX documentation gap. `AR-DEC-12` defines authority, not visual interaction. |
