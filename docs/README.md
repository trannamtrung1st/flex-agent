...
| Implementation | Sessions runtime foundation and host successor slices completed; feature status remains Partial | Executable API, Worker, SPA, Sessions module, PostgreSQL Session runtime migrations `0005`–`0029`, human-authentication migrations `0030`–`0033`, synthetic Participant Text Session, production HTTP SSE with Session-scoped reauthorization, Worker polling/Invocation processing foundations, locked restores, architecture/runtime tests, OCI build inputs, and implementation CI are present. See [workspace development](contributing/workspace.md). The Worker reference path for [ADR-016](architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) workload identity and bounded Invocation delegation is implemented and independently reviewed. The human OIDC application-session foundation is implemented and independently reviewed. Docker-backed PostgreSQL migrations through `0033`, Keycloak `26.7.0` signed back-channel logout, and NGINX restricted-route probes now have executable evidence; the remaining browser PKCE/MFA/key-rotation/clock-skew/multi-instance live matrix and full `AC-OPS-4` stay Partial. The distinct OpenRouter synthetic-development adapter, fail-closed Worker composition, deterministic contracts, and bounded live harness are implemented and independently reviewed. Live evidence remains partial: the pinned free route streamed content with validated identity, cache, metadata, and usage, but rejected strict structured control, so it is not labeled qualified and does not authorize Production, Staging, or Participant data. Gemma/Darkbloom and the Nano backup completed Phase 20 reserved structured-control probes on 2026-08-20 and both failed Decision admission without reserving content. GPT-OSS 20B/Darkbloom is the approved, separately budgeted Phase 21 candidate, but implementation and live verification are pending and no qualification label is applied. The approved OpenAI-compatible target is not yet implemented: the legacy Direct OpenAI adapter and identifiers remain in code and must stay default-off until migrated. Exact OpenAI-compatible profile qualification, private-endpoint destination-policy evidence, vLLM evidence, hosted Session creation, backup/restore, and remaining ADR-010 gates are still open. Timer polling and Invocation processing remain default-off. |
