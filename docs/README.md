...
| Implementation | Sessions runtime slice in progress (Partial) | Executable API, worker, SPA, Sessions module, PostgreSQL Session runtime migrations `0005`–`0020`, synthetic Participant Text Session, locked restores, architecture/runtime tests, OCI build inputs, and implementation CI are present. See [workspace development](contributing/workspace.md). Production HTTP SSE, live providers, Worker invocation/timer polling, OIDC application-session, backup/restore, and remaining ADR-010 gates are still open; overall scaffold acceptance and gated integrations remain blocked without that evidence. |
