...
| Implementation | Ready to begin; no P0 implementation yet | Start with the [ADR-010 workspace and contract artifacts](architecture/decisions/ADR-010-dotnet-implementation-stack-and-workspace.md#traceability-and-downstream-work). Provider-independent foundation work may proceed while affected component/provider gates are completed; no gated integration or production pilot is accepted without its required evidence. |
