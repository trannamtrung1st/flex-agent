...
| Field | Value |
| --- | --- |
| **Status** | In review v0.1 |
| **Owner** | UI/UX Lead |
| **Required approvers** | Product Lead, UI/UX Lead |
| **Version** | 0.1 |
...
This candidate is ready for product and UI/UX approval. Until approved, it is
working guidance and does not govern implementation. All modules under this
directory inherit this metadata unless they declare narrower metadata.

## Authority and boundaries

Once approved, this design system governs shared visual language, semantic
tokens, reusable component behavior, accessibility foundations, and recurring
Flex Agent UI patterns. It does not define product scope, authorize a
capability, change a lifecycle, or weaken an approved requirement.
...
The proposed visual language is **Deep-Space Operational Futurism**:
...
## Proposed shared decisions

| ID | Decision | Status | Rationale |
| --- | --- | --- | --- |
| `DS-DEC-1` | Use dark-first Deep-Space Operational Futurism as the identity-defining theme while maintaining an accessible light theme. | Proposed | Creates a distinct Agent-centered identity without making one color scheme the only operable experience. |
| `DS-DEC-2` | Use electric blue for interaction/current context and cyan only for genuine live/listening/speaking state; reserve semantic colors for their stated meanings. | Proposed | Prevents visual state collisions and decorative misuse of status colors. |
| `DS-DEC-3` | Prefer coherent work planes, rails, dividers, and inspectors over floating card stacks. | Proposed | Supports dense operational work and preserves hierarchy. |
| `DS-DEC-4` | Represent an active Agent with the abstract, stateful Agent Core rather than a generic chatbot avatar. | Proposed | Makes Agent presence distinctive without implying human identity. |
| `DS-DEC-5` | Use two density modes: interaction and workspace. | Proposed | Protects sustained reading while keeping administrative surfaces efficient. |
| `DS-DEC-6` | Treat WCAG 2.2 AA and the approved journey-level accessibility contract as the baseline for every shared pattern. | Proposed | Aligns components with the approved P0 UI/UX specifications. |
| `DS-DEC-7` | Keep protected-content and authorization state explicit and non-disclosing across loading, denial, revocation, and recovery. | Proposed | Prevents visual polish from weakening product security and privacy boundaries. |

Approval should record these decisions together or identify the decisions that
remain proposed.
...
actual system state. Long-form conversation and review content must remain calm
...
The proposed font roles name Geist Sans, Space Grotesk, and IBM Plex Mono, but
the repository does not yet contain an approved font delivery, license,
version-pinning, privacy, or fallback decision.

**Interim default:** preserve the `--font-sans`, `--font-display`, and
`--font-mono` roles but use platform fallbacks until the frontend dependency and
self-hosting approach is reviewed. This avoids an unapproved external request
or supply-chain dependency while preserving the intended hierarchy.

**Proposed decision `DS-PROP-1`:** approve self-hosted, version-pinned copies of
the named fonts after license and delivery review; never load them from a
third-party origin in an authenticated or participant surface by default.

## Versioning and change control

Use semantic design-system versions after approval. Material changes to token
