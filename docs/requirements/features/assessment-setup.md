...
- UI/UX behavior remains a traceability gap until an approved interaction specification exists and Playwright evidence covers desktop and narrow states.
...
| --- | --- | --- | --- | --- |
| `REQ-ACT-1`–`REQ-ACT-8`, `AC-ACT-1`–`AC-ACT-5`, `AC-ACT-18` | Activity draft/revision model, scoped source selectors, and optimistic concurrency — approved [MVP architecture](../../architecture/mvp-architecture.md) and [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); implementation and UI/UX specification TBD | Draft lifecycle, required-field, cross-scope, widening, stale-revision, and history tests | Empty/populated draft, selector, validation, conflict, and save states | Gap |
| `REQ-ACT-9`–`REQ-ACT-13`, `AC-ACT-4`, `AC-ACT-6` | Readiness validator using trusted ownership and immutable source metadata — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md), and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Missing/stale/mutable/cross-scope/incompatible source matrix; commit-time revalidation tests | Readiness loading, warning, blocking, recovery, and stale-source views | Gap |
| `REQ-ACT-14`–`REQ-ACT-24`, `REQ-ACT-41`, `REQ-ACT-42`, `AC-ACT-7`, `AC-ACT-8`, `AC-ACT-13`–`AC-ACT-17`, `AC-ACT-25` | Activation baseline schema/digest, atomic binding, idempotency, immutable history, and approval boundary — approved [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md) | Atomic fault injection, baseline field contract, digest fixtures, retry/concurrency, audit failure, exception-approval, and immutability tests | Confirmation, activating, exception approval, success, failure, history, and new-cohort flows | Gap |
| `REQ-ACT-25`, `REQ-ACT-31`–`REQ-ACT-34`, `AC-ACT-12`, `AC-ACT-19`, `AC-ACT-23` | Cohort/Enrollment boundary, time model, and scoped administration — approved [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md), [MVP architecture](../../architecture/mvp-architecture.md), and [`submission-attempts.md`](submission-attempts.md); implementation TBD | Empty-cohort activation, later enrollment, timezone edge, bounds, list/count isolation tests | Empty cohort, timezone validation, assignment handoff, scoped lists | Gap |
| `REQ-ACT-26`–`REQ-ACT-30`, `AC-ACT-9`–`AC-ACT-11`, `AC-ACT-26` | Stable-memory and MVP capability profile in the baseline validator — approved [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md) and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Default no-read, explicit snapshot, cross-scope memory, Dynamic write, tool/voice/shared-session negative tests | Memory choice, disabled-capability summary, error states | Gap |
| `REQ-ACT-35`–`REQ-ACT-40`, `AC-ACT-17`, `AC-ACT-20`, `AC-ACT-21` | Audit boundary, authorized baseline inspection/export, and lifecycle-policy integration — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), [ADR-003](../../architecture/decisions/ADR-003-authorization-audit-persistence.md), and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Audit schema/durability/redaction, role/scope matrix, participant denial, degraded-verification tests | Admin/reviewer summary, participant denial, audit/history views | Gap |
| UX/accessibility requirements, `AC-ACT-22` | Assessment setup interaction specification and accessible components — UI/UX spec TBD | Component accessibility and keyboard tests | Playwright desktop/narrow screenshots for all applicable states | Gap |
| Performance/reliability requirements, `AC-ACT-15`–`AC-ACT-18`, `AC-ACT-27`, `PROP-4` | Idempotency, consistency boundary, and SLO telemetry — approved [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md), [MVP architecture](../../architecture/mvp-architecture.md), and [ADR-010](../../architecture/decisions/ADR-010-dotnet-implementation-stack-and-workspace.md); implementation TBD | Load, timeout, retry, uncertain-commit reconciliation, and race tests | Pending, retry, conflict, and recovery behavior | Gap |
| Security/privacy requirements, `AC-ACT-3`, `AC-ACT-5`, `AC-ACT-21`, `AC-ACT-23`–`AC-ACT-25` | Authorization kernel plus source/baseline enforcement adapters — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md), and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Complete negative authorization/isolation, exception-approval, and sensitive-data leakage suite | Permission-denied, unavailable, loading, and redacted-summary evidence | Gap |
