...
| --- | --- | --- | --- | --- |
| `REQ-ACT-1`–`REQ-ACT-8`, `AC-ACT-1`–`AC-ACT-5`, `AC-ACT-18` | Activity draft/revision model, scoped source selectors, optimistic concurrency — architecture and UI/UX TBD | Draft lifecycle, required-field, cross-scope, widening, stale-revision, and history tests | Empty/populated draft, selector, validation, conflict, and save states | Gap |
| `REQ-ACT-9`–`REQ-ACT-13`, `AC-ACT-4`, `AC-ACT-6` | Readiness validator using trusted ownership and immutable source metadata — architecture TBD | Missing/stale/mutable/cross-scope/incompatible source matrix; commit-time revalidation tests | Readiness loading, warning, blocking, recovery, and stale-source views | Gap |
...
| `REQ-ACT-25`, `REQ-ACT-31`–`REQ-ACT-34`, `AC-ACT-12`, `AC-ACT-19`, `AC-ACT-23` | Cohort/enrollment boundary, time model, scoped administration — architecture plus `submission-attempts.md` TBD | Empty-cohort activation, later enrollment, timezone edge, bounds, list/count isolation tests | Empty cohort, timezone validation, assignment handoff, scoped lists | Gap |
| `REQ-ACT-26`–`REQ-ACT-30`, `AC-ACT-9`–`AC-ACT-11`, `AC-ACT-26` | Stable-memory and MVP capability profile in baseline validator — architecture TBD | Default no-read, explicit snapshot, cross-scope memory, Dynamic write, tool/voice/shared-session negative tests | Memory choice, disabled-capability summary, error states | Gap |
| `REQ-ACT-35`–`REQ-ACT-40`, `AC-ACT-17`, `AC-ACT-20`, `AC-ACT-21` | ADR-003 audit boundary, authorized baseline inspection/export, lifecycle policy integration — remaining architecture TBD | Audit schema/durability/redaction, role/scope matrix, participant denial, degraded-verification tests | Admin/reviewer summary, participant denial, audit/history views | Gap |
...
| Performance/reliability requirements, `AC-ACT-15`–`AC-ACT-18`, `AC-ACT-27`, `PROP-4` | Idempotency, consistency boundary, SLO telemetry — architecture TBD | Load, timeout, retry, uncertain-commit reconciliation, and race tests | Pending, retry, conflict, and recovery behavior | Gap |
| Security/privacy requirements, `AC-ACT-3`, `AC-ACT-5`, `AC-ACT-21`, `AC-ACT-23`–`AC-ACT-25` | Approved authorization kernel plus source/baseline enforcement adapters — architecture TBD | Complete negative authorization/isolation, exception-approval, and sensitive-data leakage suite | Permission-denied, unavailable, loading, and redacted-summary evidence | Gap |
