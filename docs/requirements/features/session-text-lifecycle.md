...
- Related decisions: Approved defaults `PROP-1`–`PROP-8` in this specification, with `PROP-7` revised by the 2026-08-09 product decision. [ADR-001](../../architecture/session-runtime-contract.md) governs manifest integrity and terminal sealing. [ADR-002](../../architecture/backend-module-architecture.md) governs interactive, real-time, and service authorization. [ADR-003](../../architecture/backend-module-architecture.md) governs durable audit. [ADR-005](../../architecture/session-runtime-contract.md) governs the committed readiness boundary from which this lifecycle begins. [ADR-006](../../architecture/mvp-architecture.md) governs the SPA/API/gateway, request/response plus SSE, worker, persistence, and recovery baseline. [ADR-009](../../architecture/mvp-architecture.md) approved the original detailed Session contract; [ADR-011](../../architecture/session-runtime-contract.md) supersedes its complete-message-only publication boundary and approves version 0.2 of the [text Session runtime contract](../../architecture/session-runtime-contract.md). [ADR-012](../../architecture/session-runtime-contract.md) approves the Invocation/Decision revision; [ADR-013](../../architecture/session-runtime-contract.md) approves optional next-timer replacement; [ADR-014](../../architecture/session-runtime-contract.md) approves the P0-compatible Decision-output envelope. The [MVP operational defaults](../mvp-operational-defaults.md#protected-data-lifecycle-defaults) govern the default transcript, configuration, manifest, and working-context lifecycle.
...
- Approved version 0.3 of the
  [Text Session interaction specification](../../ui-ux/flows/text-session.md)
...
| --- | --- | --- |
| `REQ-SESS-1`–`REQ-SESS-7`, `AC-SESS-1`, `AC-SESS-2` | Focused insert-readiness and ownership tests in `FlexAgent.Sessions.Tests` / Postgres end-to-end proof | Pre-start instructions, required acknowledgment, blocked, and starting states are not on the current Participant surface |
| `REQ-SESS-8`–`REQ-SESS-19`, `REQ-SESS-51`–`REQ-SESS-60`, `AC-SESS-3`–`AC-SESS-8`, `AC-SESS-31`, `AC-SESS-32`, `PROP-7` | Domain publication, rolling-validation, Postgres fragment/outbox/replay, API production SSE isolation/revalidation/subject-narrowing and cross-Session relationship tests, Runtime synthetic SSE, and web `SessionPage` tests | Desktop/narrow Playwright on the synthetic Participant journey for send, fragments, incomplete, reconnect, pause/resume, and complete |
| `REQ-SESS-61`–`REQ-SESS-85`, `AC-SESS-33`–`AC-SESS-48` | Existing `FlexAgent.Sessions.Tests`, `FlexAgent.Sessions.OpenAiCompatible.Tests` fake-transport and destination-policy contracts, architecture ownership tests, Postgres concurrency/fault/delegation/provenance tests, and `FlexAgent.Runtime.Tests` host composition and workload-identity tests; successor exact-profile qualification | Intentional no-action, timer-triggered Agent work, timer no-action, and execution-failure states on the synthetic Participant surface |
| `REQ-SESS-20`–`REQ-SESS-30`, `AC-SESS-9`–`AC-SESS-14`, `PROP-2`, `PROP-3`, `PROP-6` | Timer-lane domain, persist/due-claim, lifecycle, production SSE revalidation/subject-narrowing, enrollment-revoke-while-grant-remains, and synthetic pause/resume tests | Pause/resume, reconnecting, offline, and permission-loss states on the synthetic Participant surface; production 60-second revocation is covered by API/kernel tests, not Playwright |
| `REQ-SESS-31`–`REQ-SESS-41`, `AC-SESS-15`–`AC-SESS-20`, `PROP-5` | `ManifestTerminalizationTests` and Postgres handoff/seal fault injection | One-step Complete confirmation on the synthetic Participant surface; administrator terminate/abort UI is not present |
| `REQ-SESS-42`–`REQ-SESS-50`, `AC-SESS-21`–`AC-SESS-23`, `AC-SESS-28`–`AC-SESS-30`, `PROP-1`, `PROP-8` | Isolation and append-only repository tests; backup/restore lab not run | Participant transcript on the synthetic surface; assigned-review and administrator monitoring are not implemented |
| UX/accessibility requirements, `AC-SESS-24`–`AC-SESS-26`, `AC-SESS-31`, `AC-SESS-32` | Web unit tests plus live Playwright MCP | Desktop, 390px, 400% reflow, light/dark, and reduced-motion artifacts under `.playwright-mcp/` |
| Performance/reliability requirements, `AC-SESS-6`, `AC-SESS-9`–`AC-SESS-11`, `AC-SESS-27`, `PROP-4` | Sessions telemetry/fair-claim tests; Worker Meter sink and host-composition tests | Not a Participant-visible SLO surface |
| Security/privacy requirements, `AC-SESS-8`, `AC-SESS-13`, `AC-SESS-14`, `AC-SESS-23`, `AC-SESS-26`, `AC-SESS-29` | Wrong-ownership repository and replay-deny tests; production guessed-ID/stolen-cursor/reviewer/subject-narrowing denials; participant-in-one-Session reviewer-in-another; enrollment revoke while org grant remains; held-SSE revoke; Production-env test-identity rejection | Access denied and reconnecting on the synthetic Participant surface; production SSE isolation is API-tested |
