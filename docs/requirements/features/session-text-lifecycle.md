...
| `REQ-SESS-61`–`REQ-SESS-85`, `AC-SESS-33`–`AC-SESS-48` | Sessions domain/application, PostgreSQL runtime schema `0005`–`0024`, deterministic `IModelExecutionPort`, durable claim/lease, Decision XOR outcome, independent item validation/effect, v1 dual-read, and one-lane timer persist/due-claim coordinators exist. The Worker rehydrates immutable Session policy when Sessions persistence is configured. Timer polling requires `Sessions:TimerPolling:Enabled` plus an explicit `Sessions:WorkerServiceActorId` that already exists in IdentityAccess; the compiled default actor id is not used. Until Worker workload authentication exists, enabling timer polling outside Development/Testing refuses Worker startup; the host flag and configured actor GUID are not authentication. Durable Invocation claiming requires `Sessions:InvocationProcessing:Enabled` and is refused outside Development/Testing until invocation service delegation exists. Persistence-only and timer-only hosts do not register the live invocation work store or issue unscoped claimable-work aggregates. Timer-lane expiry fails closed; authorized renewal is deferred. Live providers remain out of scope. | `FlexAgent.Sessions.Tests`, architecture ownership tests, Postgres concurrency/fault/delegation tests, and `FlexAgent.Runtime.Tests` host composition | Intentional no-action, timer-triggered Agent work, timer no-action, and execution-failure states on the synthetic Participant surface | Partial |
