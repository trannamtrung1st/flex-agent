...
| `REQ-SESS-61`–`REQ-SESS-85`, `AC-SESS-33`–`AC-SESS-48` | Sessions domain/application, PostgreSQL runtime schema `0005`–`0024`, deterministic `IModelExecutionPort`, durable claim/lease, Decision XOR outcome, independent item validation/effect, v1 dual-read, and one-lane timer persist/due-claim coordinators exist. The Worker rehydrates immutable Session policy and, when `Sessions:TimerPolling:Enabled` is set, polls the due-timer lane under current per-Session service delegation and commit-time authorization. Live providers remain out of scope; timer polling stays default-off and is not a real-use execution profile while `FailClosedModelExecutionPort` is the only model port. | `FlexAgent.Sessions.Tests`, architecture ownership tests, Postgres concurrency/fault/delegation tests, and `FlexAgent.Runtime.Tests` host composition | Intentional no-action, timer-triggered Agent work, timer no-action, and execution-failure states on the synthetic Participant surface | Partial |
