...
| `REQ-SESS-61`–`REQ-SESS-85`, `AC-SESS-33`–`AC-SESS-48` | Sessions domain/application, PostgreSQL runtime schema `0005`–`0024`, deterministic `IModelExecutionPort`, durable claim/lease, Decision XOR outcome, independent item validation/effect, v1 dual-read, and one-lane timer persist/due-claim coordinators exist. The Worker rehydrates immutable Session policy when Sessions persistence is configured. Approved [ADR-015](../../architecture/decisions/ADR-015-session-timer-lane-service-delegation.md) governs timer delegation, and approved [ADR-016](../../architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) governs Worker workload identity and bounded Invocation delegation. Those ADR-016 runtime and deployment-profile gates are not implemented: timer polling and Invocation processing still refuse startup outside Development/Testing, and the host flag plus configured actor GUID are not authentication. Persistence-only and timer-only hosts do not register the live Invocation work store or issue unscoped claimable-work aggregates. Timer-lane expiry fails closed; authorized renewal is deferred. Live providers remain out of scope. | `FlexAgent.Sessions.Tests`, architecture ownership tests, Postgres concurrency/fault/delegation tests, and `FlexAgent.Runtime.Tests` host composition; ADR-016 workload-identity, binding, Invocation-delegation, and commit-race evidence remains pending | Intentional no-action, timer-triggered Agent work, timer no-action, and execution-failure states on the synthetic Participant surface | Partial |
