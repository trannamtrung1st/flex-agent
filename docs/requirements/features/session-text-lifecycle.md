...
| `REQ-SESS-8`–`REQ-SESS-19`, `REQ-SESS-51`–`REQ-SESS-60`, `AC-SESS-3`–`AC-SESS-8`, `AC-SESS-31`, `AC-SESS-32`, `PROP-7` | Sessions domain plus PostgreSQL persist ordered Turns, Messages, ADR-011 fragments/seals, and outbox facts. Production HTTP `GET /sessions/{id}/events` maps a trusted test actor through the ADR-002 kernel (`session.events.subscribe`) onto `ReplayAuthorizedSessionEventsCommand` over PostgreSQL and revalidates a held subscription at least every 60 seconds. OIDC product login remains deferred. Participant UI still consumes the non-authoritative synthetic `/browser` adapter. | Domain publication, rolling-validation, Postgres fragment/outbox/replay, API production SSE isolation/revalidation tests, Runtime synthetic SSE, and web `SessionPage` tests | Desktop/narrow Playwright on the synthetic Participant journey for send, fragments, incomplete, reconnect, pause/resume, and complete | Partial |
...
| Security/privacy requirements, `AC-SESS-8`, `AC-SESS-13`, `AC-SESS-14`, `AC-SESS-23`, `AC-SESS-26`, `AC-SESS-29` | Composite-scope PostgreSQL repositories, browser-safe projections, synthetic access-revoke, and production SSE ADR-002 kernel with 60-second revalidation for trusted test actors. Full XSS/rate/export isolation matrix and OIDC remain later. | Wrong-ownership repository and replay-deny tests; production guessed-ID/stolen-cursor/reviewer denials; held-SSE revoke | Access denied and reconnecting on the synthetic Participant surface; production SSE isolation is API-tested | Partial |
