...
| `REQ-SESS-8`–`REQ-SESS-19`, `REQ-SESS-51`–`REQ-SESS-60`, `AC-SESS-3`–`AC-SESS-8`, `AC-SESS-31`, `AC-SESS-32`, `PROP-7` | Sessions domain plus PostgreSQL persist ordered Turns, Messages, ADR-011 fragments/seals, and outbox facts. Production HTTP `GET /sessions/{id}/events` is an adapter over `ReplayAuthorizedSessionEventsCommand` and the ADR-002 kernel (`session.events.subscribe`): it resolves the actor's current Session-scoped relationship from trusted records (session → organization → activity → participant/enrollment → relationship), rehydrates frozen-policy bindings, and revalidates that chain plus org grant at least every 60 seconds. Production human identity is the OIDC application-session cookie; Development/Testing may still use a harness key plus actor id. Participant UI still uses synthetic `/browser`. | Domain publication, rolling-validation, Postgres fragment/outbox/replay, API production SSE isolation/revalidation/subject-narrowing and cross-Session relationship tests, Runtime synthetic SSE, and web `SessionPage` tests | Desktop/narrow Playwright on the synthetic Participant journey for send, fragments, incomplete, reconnect, pause/resume, and complete | Partial |
| `REQ-SESS-61`–`REQ-SESS-85`, `AC-SESS-33`–`AC-SESS-48` | Sessions domain/application, PostgreSQL runtime schema `0005`–`0029`, deterministic `IModelExecutionPort`, legacy Direct OpenAI official-SDK adapter isolated in `FlexAgent.Sessions.OpenAi`, durable claim/lease with control-and-stream heartbeat, currently authorized claim-fenced provider-request `started` reservation before network I/O, Decision XOR outcome, independent item validation/effect, v1 dual-read, and one-lane timer persist/due-claim coordinators exist. Frozen provider-profile and opaque credential-binding references rehydrate with the trusted Session binding. ADR-008 and ADR-010 now target the vendor-neutral `openai_compatible` / `sessions.openai_compatible.v1` contract; implementation and durable-identity migration are later work and may complete through deterministic evidence without a selected live profile. The Worker keeps live model execution default-off and fail-closed, and neither the legacy adapter nor deterministic migration evidence qualifies the new target. Approved [ADR-015](../../architecture/decisions/ADR-015-session-timer-lane-service-delegation.md) governs timer delegation. Approved [ADR-016](../../architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) is implemented and independently reviewed for Worker signed-JWT workload identity, principal binding, recoverable versus shutdown gates, passive readiness, and bounded `session.invocation.execute` claim/disclosure/commit authorization. Production/Staging still require an approved deployment profile and a live issuer before enablement; host flags remain default `false`. Historical Invocation work without a delegation envelope stays unclaimable. Persistence-only and timer-only hosts do not register the live Invocation work store or issue unscoped claimable-work aggregates. Timer-lane expiry fails closed; authorized renewal is deferred. Exact OpenAI-compatible profile qualification remains a separate successor gate; Organization-hosted private-endpoint live evidence and synthetic OpenRouter/vLLM contract evidence remain outside this slice. | Existing `FlexAgent.Sessions.Tests`, legacy `FlexAgent.Sessions.OpenAi.Tests` fake-transport contracts, architecture ownership tests, Postgres concurrency/fault/delegation/provenance tests, and `FlexAgent.Runtime.Tests` host composition and workload-identity tests; later OpenAI-compatible migration, deterministic private-destination negative tests, and successor exact-profile qualification | Intentional no-action, timer-triggered Agent work, timer no-action, and execution-failure states on the synthetic Participant surface | Partial |
| `REQ-SESS-20`–`REQ-SESS-30`, `AC-SESS-9`–`AC-SESS-14`, `PROP-2`, `PROP-3`, `PROP-6` | Authoritative pause/resume, active-time timer lane, and cutoff cancellation persist. The production SSE adapter revalidates the current Session-scoped subject relationship and org grant, or terminates a held `/sessions/{id}/events` subscription, within 60 seconds. Configurable warning schedule remains later. Production SSE reconnect must present a current application session; synthetic UI SSE reconnect is still harness-only. | Timer-lane domain, persist/due-claim, lifecycle, production SSE revalidation/subject-narrowing, enrollment-revoke-while-grant-remains, and synthetic pause/resume tests | Pause/resume, reconnecting, offline, and permission-loss states on the synthetic Participant surface; production 60-second revocation is covered by API/kernel tests, not Playwright | Partial |
...
| Performance/reliability requirements, `AC-SESS-6`, `AC-SESS-9`–`AC-SESS-11`, `AC-SESS-27`, `PROP-4` | Bounded sanitizing Session telemetry, Organization/Activity fair claiming, and a claimable-work index exist. Sampled invocation backlog gauges run only when the Development/Testing invocation-processing profile is composed; they are not an authorization-exempt operator read of persistence-only hosts. p95 admission/reconnect load, timer-storm, and OTLP/Collector export remain later. | Sessions telemetry/fair-claim tests; Worker Meter sink and host-composition tests | Not a Participant-visible SLO surface | Partial |
| Security/privacy requirements, `AC-SESS-8`, `AC-SESS-13`, `AC-SESS-14`, `AC-SESS-23`, `AC-SESS-26`, `AC-SESS-29` | Composite-scope PostgreSQL repositories, browser-safe projections, synthetic access-revoke, and a production SSE adapter that resolves Session-scoped current relationship plus ADR-002 grant every 60 seconds. Test identity is disabled outside Development/Testing and requires a harness key; an actor GUID alone is not authentication. Held SSE revalidates the application session and ADR-002 grant within 60 seconds. Full XSS/rate/export isolation matrix and product UI adoption remain later. | Wrong-ownership repository and replay-deny tests; production guessed-ID/stolen-cursor/reviewer/subject-narrowing denials; participant-in-one-Session reviewer-in-another; enrollment revoke while org grant remains; held-SSE revoke; Production-env test-identity rejection | Access denied and reconnecting on the synthetic Participant surface; production SSE isolation is API-tested | Partial |
