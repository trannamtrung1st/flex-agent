...
| `REQ-SESS-8`–`REQ-SESS-19`, `REQ-SESS-51`–`REQ-SESS-60`, `AC-SESS-3`–`AC-SESS-8`, `AC-SESS-31`, `AC-SESS-32`, `PROP-7` | Sessions domain plus PostgreSQL persist ordered Turns, Messages, ADR-011 fragments/seals, and outbox facts. Production HTTP `GET /sessions/{id}/events` is an adapter seam over `ReplayAuthorizedSessionEventsCommand` and the ADR-002 kernel (`session.events.subscribe`): it revalidates current subject relationship plus org grant at least every 60 seconds. Hosted bindings fail closed until frozen-policy rehydration exists. The adapter's actor-keyed subject lookup is not the production authorization model; persistence-backed Session-scoped chain resolution (session → organization → activity → participant/enrollment → current relationship) remains later. Test identity is a Development/Testing harness key plus actor id, never a production login. OIDC and participant UI remain later; UI still uses synthetic `/browser`. | Domain publication, rolling-validation, Postgres fragment/outbox/replay, API production SSE isolation/revalidation/subject-narrowing tests, Runtime synthetic SSE, and web `SessionPage` tests | Desktop/narrow Playwright on the synthetic Participant journey for send, fragments, incomplete, reconnect, pause/resume, and complete | Partial |
...
| Security/privacy requirements, `AC-SESS-8`, `AC-SESS-13`, `AC-SESS-14`, `AC-SESS-23`, `AC-SESS-26`, `AC-SESS-29` | Composite-scope PostgreSQL repositories, browser-safe projections, synthetic access-revoke, and a production SSE adapter that revalidates current subject relationship plus ADR-002 grant every 60 seconds. Test identity is disabled outside Development/Testing and requires a harness key; an actor GUID alone is not authentication. Actor-keyed subject lookup is adapter-only. Full XSS/rate/export isolation matrix, Session-scoped subject/binding rehydration, and OIDC remain later. | Wrong-ownership repository and replay-deny tests; production guessed-ID/stolen-cursor/reviewer/subject-narrowing denials; held-SSE revoke; Production-env test-identity rejection | Access denied and reconnecting on the synthetic Participant surface; production SSE isolation is API-tested | Partial |
