...
- Last amended: 2026-08-23 — approved `PROP-8`, `REQ-SUBM-57`–`REQ-SUBM-58`,
  ...
  so the 10-second window cannot change and cleanup is expiry-indexed.
...
| Performance/reliability requirements, `REQ-SUBM-57`–`REQ-SUBM-58`, `AC-SUBM-9`, `AC-SUBM-17`, `AC-SUBM-27`, `AC-SUBM-40`–`AC-SUBM-41`, `PROP-5`, `PROP-8` | Enrollment mutation idempotency, coordinator consistency, bounded mutation/request-limit telemetry, replica-local defense in depth, and PostgreSQL-backed replica-independent request admission are implemented. Policy `window_seconds` is frozen at 10 seconds so a duration change cannot reset the shared budget; counters store indexed `expires_at` for bounded `SKIP LOCKED` cleanup. Attempt eligibility, metadata finalization, hosted load labs, and a true two-OS-process lab remain TBD. | Two-port in-process aggregate-budget, database-clock window/cleanup, frozen-window rejection, expiry-index cleanup, policy-mismatch, lock-timeout fail-closed, restart, recovery, and representative admission-latency tests plus HTTP 429/503 contract tests | Existing recoverable 429 UI remains applicable; shared-admission 503 uses the existing protected unavailable state and adds no new interaction contract | Partial |
