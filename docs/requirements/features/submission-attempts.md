...
- Related UI/UX: The approved [Submission and Attempt interaction specification](../../ui-ux/submission-attempt.md) governs administrator Enrollment and exception-approval interaction plus Participant Submission, accepted-version, Attempt-readiness, start, reconciliation, accessibility, responsive, and protected-content behavior. Administrator Participant and participant-option lists use signed server cursors (`UI-SUBM-DEC-13`); the browser must not load every row to exclude already-enrolled identities.
