...
- Related UI/UX: The approved [Submission and Attempt interaction specification](../../ui-ux/flows/submission-attempt.md) governs administrator Enrollment and exception-approval interaction plus Participant Submission, accepted-version, Attempt-readiness, start, reconciliation, accessibility, responsive, and protected-content behavior. Administrator Participant and participant-option lists use signed server cursors (`UI-SUBM-DEC-13`); the browser must not load every row to exclude already-enrolled identities.
- Related decisions: [ADR-001](../../architecture/session-runtime-contract.md) governs compatible protected references from the resolved configuration and manifest. [ADR-002](../../architecture/backend-module-architecture.md) and [ADR-003](../../architecture/backend-module-architecture.md) govern authorization enforcement and durable audit. [ADR-005](../../architecture/session-runtime-contract.md) governs the atomic attempt/session-start and exact Submission-binding boundary required by `PROP-2`. [ADR-006](../../architecture/mvp-architecture.md) governs quarantine and immutable artifact storage. [ADR-008](../../operations/README.md) selects the object-store default and configurable `ArtifactSafetyScanner` boundary, while the [MVP operational defaults](../mvp-operational-defaults.md) govern intake, authentication-session, lifecycle, and recovery defaults. `PROP-1`–`PROP-7` were approved on 2026-08-06; `PROP-8`–`PROP-15` and [ADR-018](../../architecture/backend-module-architecture.md) were approved on 2026-08-23 and are recorded under [Approved decision dispositions](#approved-decision-dispositions).
...
- WCAG 2.2 AA is the contractual target under the approved
  [Submission and Attempt interaction specification](../../ui-ux/flows/submission-attempt.md).
...
- Approved authentication, authorization, resource-isolation, scoped-query, revocation, commit-time reauthorization, and audit-durability behavior from [`auth-resource-isolation.md`](auth-resource-isolation.md), [ADR-002](../../architecture/backend-module-architecture.md), and [ADR-003](../../architecture/backend-module-architecture.md).
- Activated cohort baseline, versioned task/submission requirement, attempt limit, timing/deadline rules, participant-accommodation bounds, and immutable source references from [`assessment-setup.md`](assessment-setup.md) and [ADR-004](../../architecture/mvp-architecture.md).
...
- Apply the approved [Submission and Attempt interaction specification](../../ui-ux/flows/submission-attempt.md). Frontend implementation, automated accessibility checks, and Playwright evidence across desktop and narrow states remain delivery gaps.
...
| `Q-2` / `PROP-2` | Bind the exact accepted version set in the authoritative attempt/session-start consistency boundary; fail start without consumption if binding cannot commit. Later in-session versions require the frozen rule and explicit ordered bindings. | Prevents a moving evidence target. [ADR-005](../../architecture/session-runtime-contract.md) governs the implementation boundary. |
...
| --- | --- | --- |
| `REQ-SUBM-1`–`REQ-SUBM-8`, `REQ-SUBM-43`, `AC-SUBM-1`–`AC-SUBM-4`, `AC-SUBM-29`, `PROP-6` | Domain, PostgreSQL assignment/close/reassign, contract catalog, HMAC-scoped cursors, and HTTP CSRF/unknown-member/concealed-detail/rate-limit tests | Component empty/active/suspended/conflict/confirm states. Authenticated Playwright desktop/narrow/both-theme evidence exists; 400% zoom remains a recorded gap |
| `REQ-SUBM-9`–`REQ-SUBM-14`, `REQ-SUBM-50`–`REQ-SUBM-56`, `AC-SUBM-11`, `AC-SUBM-12`, `AC-SUBM-22`, `AC-SUBM-33`–`AC-SUBM-39`, `PROP-3`, `PROP-9`–`PROP-15` | Domain coordinators; PostgreSQL `0046`/`0047` upgrade, isolation, parent-scope negative inserts, concurrency, idempotency, session-revocation, audit-rollback, and append-only tests; contract catalog/OpenAPI `$ref`/C#/TypeScript v2 fixtures including decide/revoke; HTTP CSRF/unknown-member/unauthenticated timing negatives; Enrollment and **My work** vitest | Component timing/accommodation states exist. Authenticated Playwright MCP desktop/narrow/400%/theme evidence is not yet captured for this slice |
| `REQ-SUBM-15`–`REQ-SUBM-23`, `AC-SUBM-5`–`AC-SUBM-10`, `PROP-1` | Limit, readiness, pre/post-commit fault injection, separately authorized exception approval/rejection, mandatory requester/approver separation, retry, timeout, and multiple-device/concurrent start tests | Eligible, confirmation, approval-required, exception decision/reconciliation, starting, active, exhausted, failure, uncertain/reconciled, and aborted states |
| `REQ-SUBM-24`–`REQ-SUBM-35`, `REQ-SUBM-44`–`REQ-SUBM-48`, `AC-SUBM-13`–`AC-SUBM-18`, `AC-SUBM-30`–`AC-SUBM-32`, `PROP-2`, `PROP-4` | Domain policy/intake/capability/cleanup including Activity-closure eligibility, delete-failure/exact-version, missing-version fail-closed, duplicate-claim no-op, scan-cursor CAS, and held-page scan-cursor regressions; PostgreSQL `0048`–`0060` parent-scope, immutability, concurrent finalize, durable work exact artifact version backfill, unbackfillable reconstruction, disposition acquisition guards, accepted-overlap reconstruction, lifecycle hold/disposition, and `cleanup_accepted` work kind; SeaweedFS conditional/presign/scope/restore/expiry/exact-version delete; HTTP CSRF/admission/unauthenticated `no-store` including version detail; React **My work** intake/history/permission-loss/multi-item preview/reconciling-refresh tests; Dapper accepted-version list/find materialization | Component empty/editing/submit/cancel/permission-loss/history-when-unavailable/multi-item/reconciling states. Authenticated Playwright on rebuilt compose: local prepare, confirm, accept, later version, inert preview, session download, 360px, CSS `zoom: 4` (sticky chrome dominates; not a WCAG reflow pass). Live receiving plus **Cancel intake** during receiving, cancelled recovery, Version 3 inert preview, and sign-out from preview captured 2026-08-25. Live validating captured 2026-08-25 by delaying finalize: desktop `.playwright-mcp/page-2026-08-25T10-42-30-790Z.png`, 360px `.playwright-mcp/page-2026-08-25T10-41-57-464Z.png`; Version 4 then accepted |
| `REQ-SUBM-36`–`REQ-SUBM-42`, `REQ-SUBM-49`, `AC-SUBM-19`–`AC-SUBM-21`, `AC-SUBM-24`–`AC-SUBM-26`, `PROP-7` | Preview audit-before-disclosure and audit-unavailable fail-closed tests; unauthenticated preview/download/version-detail `no-store`; SeaweedFS download-presign expiry and scope isolation; catalog/OpenAPI/C#/TypeScript parity for `AcceptedVersionDetailV2` and `ProtectedItemPreviewV2` | Permission-loss focus and inert viewer component evidence exists; rebuilt-compose inert preview of Version 2, Version 3, and Version 4 (2026-08-25 recheck, `.playwright-mcp/page-2026-08-25T10-48-00-927Z.png`) plus sign-out from an open preview captured 2026-08-25. Assigned-review workspace and administrator scoped history remain Gap |
| UX/accessibility requirements, `AC-SUBM-23` | Component keyboard/focus/dialog/error-summary/multi-item/reconciling tests | Authenticated Playwright desktop, dark theme, 360px, 320px, CSS `zoom: 4`, live receiving with **Cancel intake**, cancelled recovery, live validating (desktop and 360px, 2026-08-25T10:41–10:42Z), and sign-out from exact preview exist under `.playwright-mcp/`. Sticky chrome at 400% zoom is not a reflow pass |
| Performance/reliability requirements, `REQ-SUBM-57`–`REQ-SUBM-58`, `AC-SUBM-9`, `AC-SUBM-17`, `AC-SUBM-27`, `AC-SUBM-40`–`AC-SUBM-41`, `PROP-5`, `PROP-8` | Two-port in-process aggregate-budget, database-clock window/cleanup, frozen-window rejection, expiry-index cleanup, mutated-`0044` longer-window freeze-in-place, live old-window refusal, overlap-horizon refusal until the aligned deployed-policy bucket ends (including non-divisible `12 → 20`), policy-mismatch, lock-timeout fail-closed, restart, recovery, and representative admission-latency tests plus HTTP 429/503 contract tests | Existing recoverable 429 UI remains applicable; shared-admission 503 uses the existing protected unavailable state and adds no new interaction contract |
| Security/privacy requirements, `AC-SUBM-14`, `AC-SUBM-18`–`AC-SUBM-21`, `AC-SUBM-26`, `AC-SUBM-28` | Isolation, capability mismatch/expiry, malware-scanner fail-closed, HTTP negatives, artifact scope tests | Permission-denied and unavailable preview component evidence; full malicious-content Playwright suite remains outstanding |
