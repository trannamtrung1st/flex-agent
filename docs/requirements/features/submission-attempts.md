...
| Performance/reliability requirements, `AC-SUBM-9`, `AC-SUBM-17`, `AC-SUBM-27`, `PROP-5` | Enrollment mutation idempotency, coordinator consistency, bounded mutation/request-limit telemetry, and replica-local per-actor/Organization Enrollment request limits. Attempt eligibility, metadata finalization, and hosted load labs remain TBD. | PostgreSQL 20-sample assignment mutation p95 and in-service Enrollment authorization p95; HTTP 429 after the frozen read quota | Progress, cancel, retry, delayed validation, uncertain outcome, recovery states for later Submission/Attempt work remain unimplemented | Partial |
