...
| `REQ-AUTH-11`, `REQ-AUTH-15`, `AC-AUTH-16` | Service identity and delegated background execution — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); approved [ADR-015](../../architecture/decisions/ADR-015-session-timer-lane-service-delegation.md) realizes the Session timer delegation. Approved [ADR-016](../../architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) is implemented for the Worker reference path: signed-JWT validation, mounted-file client secret, versioned principal binding with provision/revoke/replace, recoverable identity gate, additive schema `0025`, and per-Session `session.invocation.execute` envelopes on claim/commit. Production and Staging compose protected lanes only with the OAuth JWT profile; both host flags remain default `false`. Deployment-profile numeric bounds, live issuers, and live model providers are not qualified. | Job/event scope tampering, workload-authentication, binding, cross-scope, and stale-delegation tests | Operational evidence only | Partial |
