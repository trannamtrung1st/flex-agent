...
| --- | --- | --- | --- | --- |
| `REQ-AUTH-1`, `REQ-AUTH-2`, `AC-AUTH-1`, `AC-AUTH-13` | Trusted identity and authorization boundary — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md). Human login resolves an exact pre-provisioned `(issuer, subject)` binding to one enabled actor and one server-derived Organization, then issues a PostgreSQL-backed opaque application session (`0030`–`0033`). Back-channel logout scopes `sid`, `sub`, and `sid+sub` per the [Keycloak OIDC contract](../../operations/provider-profiles/keycloak-oidc-contract.md). Provider roles are not authorization. Production feature APIs beyond current-session, logout, lifecycle, and protected SSE remain later. The code change is independently reviewed; Docker-backed PostgreSQL/`0033` and live Keycloak evidence remain open. | Identity-resolution, Organization-context, cookie/OIDC contract, back-channel scope, JWKS, and SSE application-session tests | Product UI still uses the synthetic `/browser` adapter | Partial |
