...
| `REQ-AUTH-11`, `REQ-AUTH-15`, `AC-AUTH-16` | Service identity and delegated background execution — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); approved [ADR-015](../../architecture/decisions/ADR-015-session-timer-lane-service-delegation.md) realizes the Session timer delegation, and approved [ADR-016](../../architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) governs Worker workload identity and bounded Invocation delegation. ADR-016 implementation and deployment-profile evidence remain open. | Job/event scope tampering, workload-authentication, binding, cross-scope, and stale-delegation tests | Operational evidence only | Gap |
