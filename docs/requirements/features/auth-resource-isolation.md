...
| --- | --- | --- | --- | --- |
| `REQ-AUTH-1`, `REQ-AUTH-2`, `AC-AUTH-1`, `AC-AUTH-13` | Trusted identity and authorization boundary — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md). Human login resolves an exact pre-provisioned `(issuer, subject)` binding to one enabled actor and one server-derived Organization, then issues a PostgreSQL-backed opaque application session (`0030`–`0033`). Back-channel logout scopes `sid`, `sub`, and `sid+sub` per the [Keycloak OIDC contract](../../operations/provider-profiles/keycloak-oidc-contract.md). Provider roles are not authorization. Production feature APIs beyond current-session, logout, lifecycle, Assessment shell reads used by the auth journey, and protected SSE remain later. The code change is independently reviewed. Docker-backed PostgreSQL/`0033`, Keycloak `26.7.0` logout-token compatibility (`FlexAgent.Keycloak.Integration.Tests`), canonical Compose/NGINX, and `pnpm verify:oidc` Playwright (`OIDC-E2E-01`–`07`) have executable evidence. Remaining real MFA, key rotation, clock skew, account-disablement, outage, and multi-instance live matrix stay later. | Identity-resolution, Organization-context, cookie/OIDC contract, back-channel scope, JWKS, PostgreSQL persistence, SSE application-session tests, and required `pnpm verify:oidc` | Canonical `web-legacy` gateway journey plus named candidate/non-Production Wave 8.1 shell; synthetic `/browser` is not authentication evidence | Partial |
| `REQ-AUTH-3`–`REQ-AUTH-6`, `AC-AUTH-4`, `AC-AUTH-5`, `AC-AUTH-10` | Organization ownership and delegated-scope enforcement — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md) and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Cross-organization matrix; upper/lower-scope conflict tests | Administrator scope and denial flows | Gap |
| `REQ-AUTH-7`, `REQ-AUTH-8`, `REQ-AUTH-24`, `AC-AUTH-2`, `AC-AUTH-3`, `AC-AUTH-19` | Enrollment, Participant ownership, and Session isolation — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md) and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Cross-participant and concurrent-session integration tests | Participant own-session and denied-resource journeys | Gap |
| `REQ-AUTH-9`, `REQ-AUTH-10`, `AC-AUTH-5`, `AC-AUTH-6` | Reviewer and administrator assignments — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); implementation TBD | Active, expired, revoked, and unrelated assignment tests | Reviewer assigned/unassigned states | Gap |
| `REQ-AUTH-11`, `REQ-AUTH-15`, `AC-AUTH-16` | Service identity and delegated background execution — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); approved [ADR-015](../../architecture/decisions/ADR-015-session-timer-lane-service-delegation.md) realizes the Session timer delegation. Approved [ADR-016](../../architecture/decisions/ADR-016-worker-workload-identity-and-invocation-delegation.md) is implemented and independently reviewed for the Worker reference path: signed-JWT validation, mounted-file client secret, versioned principal binding with provision/revoke/replace, locking claim/commit reauthorization, model-disclosure admission before `ExecuteAsync` and stream start, recoverable identity gate with passive readiness, additive schema `0025`, and per-Session `session.invocation.execute` envelopes. Production and Staging compose protected lanes only with the OAuth JWT profile; both host flags remain default `false`. Deployment-profile numeric bounds, live issuers, and live model providers are not qualified. | Job/event scope tampering, workload-authentication, binding, cross-scope, stale-delegation, claim/revoke, and disclosure-admission tests | Operational evidence only | Partial |
| `REQ-AUTH-12`–`REQ-AUTH-14`, `AC-AUTH-7`, `AC-AUTH-8` | Scoped queries, linked-resource validation, and artifact delivery — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), [MVP architecture](../../architecture/mvp-architecture.md), and [ADR-008](../../architecture/decisions/ADR-008-bounded-oss-component-set.md). Worker persistence-only and timer-only hosts do not register the live invocation work store or issue an unscoped claimable-work aggregate; that count is not an authorization-exempt operator read. Remaining list/count/export paths remain TBD. | List/count leakage; parent mismatch; download/export tests; Worker host composition | Scoped tables, search, empty states, and denied downloads | Gap |
| `REQ-AUTH-16`, `REQ-AUTH-17`, `AC-AUTH-9` | Trusted parent-derived ownership — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); implementation TBD | Forged organization/owner field contract tests | Validation and denial messages | Gap |
...
| `REQ-AUTH-21`, `REQ-AUTH-22`, `AC-AUTH-3`, `AC-AUTH-13` | Non-disclosing denial contract — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md); implementation TBD | Existence-oracle and no-side-effect tests | Generic unavailable states | Gap |
| `REQ-AUTH-23`, `AC-AUTH-18` | Result visibility enforcement supplied by the approved [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Released/unreleased participant-access tests | Participant result visibility states | Gap |
...
| `REQ-AUTH-32`, `AC-AUTH-23` | Completed-resource relationship, workflow, visibility, and Release enforcement — approved [text Session runtime](../../architecture/session-runtime-contract.md), [Evaluation execution](../../architecture/evaluation-execution-contract.md), and [Review/Release](../../architecture/review-result-release-contract.md) contracts; implementation TBD | Completed-resource active/revoked relationship and released/unreleased visibility tests | Completed-session participant and reviewer access states | Gap |
| `REQ-AUTH-30`, `AC-AUTH-21` | Named OIDC positive, anonymous, unbound, ambiguous-Organization, revoked-session, and public-route cases execute in the required `oidc` CI job (`pnpm verify:oidc`). Per-resource wrong-participant, wrong-assignment, forged-ID, list/count, and cross-session matrices remain owned by feature suites. | Machine-readable Playwright case-ID manifest plus Keycloak compatibility and Compose negatives | Canonical and candidate Playwright reports under `artifacts/oidc/` | Partial |
| UX and accessibility requirements, `AC-AUTH-20` | The approved [Activity/Campaign journey](../../ui-ux/activity-campaign-journey.md), [assessment Campaign setup](../../ui-ux/assessment-campaign-setup.md), [Submission and Attempt](../../ui-ux/submission-attempt.md), [Text Session](../../ui-ux/text-session.md), [Evidence, Evaluation, and Human Review](../../ui-ux/evidence-evaluation-human-review.md), and [Result and Release](../../ui-ux/result-release.md) specifications define current P0 access-state patterns; implementation remains TBD | Component accessibility tests where applicable | Keyboard, focus, narrow viewport, loading, and denial screenshots | Gap |
