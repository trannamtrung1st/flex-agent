...
| --- | --- | --- | --- | --- |
| `REQ-AUTH-1`, `REQ-AUTH-2`, `AC-AUTH-1`, `AC-AUTH-13` | Trusted identity and authorization boundary — approved [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md). Human login now resolves an exact pre-provisioned `(issuer, subject)` binding to one enabled actor and one server-derived Organization, then issues a PostgreSQL-backed opaque application session. Provider roles are not authorization. Production feature APIs beyond current-session, logout, lifecycle, and protected SSE remain later. | Identity-resolution, Organization-context, cookie/OIDC contract, and SSE application-session tests | Product UI still uses the synthetic `/browser` adapter | Partial |
