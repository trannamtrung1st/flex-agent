...
| --- | --- | --- | --- | --- |
| `REQ-RSC-1`–`REQ-RSC-8`, `AC-RSC-1`–`AC-RSC-5` | Resolver boundary, precedence engine, trusted ownership chain, and override validation — approved [ADR-001](../../architecture/decisions/ADR-001-resolved-configuration-representation-and-integrity.md), [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Unit/property tests for precedence; cross-layer conflict matrix; unauthorized/authorized override tests | Administrator conflict and override states; reviewer provenance summary | Gap |
| `REQ-RSC-9`–`REQ-RSC-14`, `AC-RSC-6`, `AC-RSC-8`, `AC-RSC-24` | Cohort-baseline validator and assessment fairness profile — approved [ADR-004](../../architecture/decisions/ADR-004-assessment-activation-baseline-and-atomicity.md) and [MVP architecture](../../architecture/mvp-architecture.md); implementation TBD | Baseline drift, cross-cohort substitution, memory snapshot/no-read, deferred-capability tests | Cohort readiness and start-blocked flows | Gap |
...
| `REQ-RSC-29`–`REQ-RSC-37`, `AC-RSC-9`, `AC-RSC-14`, `AC-RSC-15`, `AC-RSC-17`, `AC-RSC-23` | Sessions persist append-only runtime manifest references and a `manifest-jcs-sha256-v2` terminal seal with eligible Evaluation handoff for `Completed`. The full ADR-005 Attempt/Submission start field set and production configuration-source registry remain outside this slice. Pre-`0017` terminal rows stay unsealed. | `ManifestTerminalizationTests`; Postgres `0017`/`0018` seal and handoff fault injection | No reviewer manifest UI | Partial |
| `REQ-RSC-46`, `AC-RSC-25` | Opaque Organization-scoped or deployment-default credential-binding identity is frozen on the trusted Session binding and revalidated against an installed profile plus mounted-file secret at execution. Worker-global `Sessions:ModelDeployment:*` settings cannot replace that frozen authority. The approved adapter target is the vendor-neutral `openai_compatible` contract behind `IModelExecutionPort`; the current `FlexAgent.Sessions.OpenAi` Direct OpenAI adapter is a legacy implementation that remains default-off and requires later deterministic migration. That migration may complete without a selected live profile, but it does not qualify or enable the adapter. No raw credential enters Session records, contracts, or browser artifacts. Exact-profile live qualification remains a separate successor gate, and Organization-hosted private-endpoint live evidence remains open. | Domain/port no-fallback tests; frozen-authority processor tests; fake-transport adapter tests; secret traversal/symlink/size tests; later adapter-identity migration and deterministic private-destination negative tests; successor exact-profile qualification | No Participant credential UI; binding status is not a browser field | Partial |
| `REQ-RSC-47`–`REQ-RSC-55`, `AC-RSC-26`–`AC-RSC-28` | Immutable `FrozenTextSessionRuntimePolicy` with P0 capability kernel, timer-lane bounds, non-widening merge, `rsc-jcs-sha256-v1` digest, and historical v1 Decision reconstruction. Production source-layer freeze at Activity/cohort activation remains later; Session bind uses trusted test fixtures plus persisted runtime policy. | Frozen-policy resolver tests; contract v1/v2 dual-read; Sessions Decision tests | Participant UI hides envelope, output-id, audience, and timer internals | Partial |
