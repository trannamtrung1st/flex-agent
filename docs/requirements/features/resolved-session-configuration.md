...
| `REQ-RSC-46`, `AC-RSC-25` | Opaque Organization-scoped or deployment-default credential-binding identity is frozen on the trusted Session binding and revalidated against an installed profile plus mounted-file secret at execution. Worker-global `Sessions:ModelDeployment:*` settings cannot replace that frozen authority. Direct OpenAI lives in `FlexAgent.Sessions.OpenAi` behind `IModelExecutionPort` and stays fail-closed until an operator-qualified profile is installed. No raw credential enters Session records, contracts, or browser artifacts. Exact-profile live qualification remains open. | Domain/port no-fallback tests; frozen-authority processor tests; fake-transport adapter tests; secret traversal/symlink/size tests | No Participant credential UI; binding status is not a browser field | Partial |
