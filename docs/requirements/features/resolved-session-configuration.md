...
| `REQ-RSC-46`, `AC-RSC-25` | Opaque Organization-scoped credential-binding identity is resolved in Sessions and fail-closed at the deterministic model-execution port. No live provider, secret material, or payer fallback is implemented. Credentials do not enter Session records, contracts, or browser artifacts. | Domain/port no-fallback tests; fake adapter preflight | No Participant credential UI; binding status is not a browser field | Partial |
