...
- WCAG 2.2 AA is the approved requirements baseline under `PROP-9`. Review-side interaction is approved in the [Evidence, Evaluation, and Human Review interaction specification](../../ui-ux/evidence-evaluation-human-review.md); a separate Result and Release interaction specification must define the remaining Release and Participant-facing contract.
...
- Approved [Evidence, Evaluation, and Human Review interaction specification](../../ui-ux/evidence-evaluation-human-review.md) for review queues, criterion/Evidence inspection, Human revision, Review decision, pre-decision preview, conflict, and Reviewer accessibility states. An approved Result and Release interaction specification remains required for Release confirmation, Participant Result, correction, and their accessibility states.
...
| `Q-9`, `PROP-9` | Use WCAG 2.2 AA as the requirements baseline across desktop and narrow layouts. | Provides a testable inclusive baseline while detailed interactions remain owned by the approved review-side and remaining Result/Release UI/UX specifications. |
...
| --- | --- | --- | --- | --- |
| `REQ-REV-1`–`REQ-REV-8`, `AC-REV-1`–`AC-REV-4`, `PROP-1` | Review-case/assignment state machine, trusted handoff, candidate selector, and scoped queue — architecture approved in the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Eligibility, no-Evaluation, exact-version, replacement, stale candidate, wrong-scope list/count, claim/reassign/revoke concurrency tests | Awaiting, ready, assigned, stale, unresolved, denied states | Gap |
| `REQ-REV-9`–`REQ-REV-18`, `AC-REV-5`–`AC-REV-9`, `PROP-2` | Authorized inspection adapters, revision schema/validator, content classification, and rendering — architecture approved in the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation and machine-readable schema TBD | Source reauthorization, field allowlist/range, Evidence link, immutability, note leakage, unsafe-content, chain-of-thought exclusion tests | Evaluation/Evidence inspection, revision form, validation, focus return, internal/participant content separation | Gap |
| `REQ-REV-19`–`REQ-REV-26`, `AC-REV-6`, `AC-REV-10`–`AC-REV-15` | Decision state machine, expected-version/idempotency boundary, immutable lineage, and audit — architecture approved in [ADR-003](../../architecture/decisions/ADR-003-authorization-audit-persistence.md) and the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Approve unchanged, revised approve, reject, escalate, stale permission, concurrent/duplicate/conflicting decision, audit failure tests | Decision confirmations, conflict/recovery, approved/rejected/escalated states | Gap |
| `REQ-REL-1`–`REQ-REL-5`, `AC-REL-1`, `AC-REL-2`, `PROP-3` | Versioned Result schema, Participant projection, and protected-content/rendering validators — architecture approved in the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation and machine-readable schema TBD | Allowlist/completeness/range/aggregation, internal-field leakage, locale, markup/link, integrity tests | Result preview, validation, unavailable and unsafe-content states | Gap |
| `REQ-REL-6`–`REQ-REL-13`, `AC-REL-3`–`AC-REL-12`, `PROP-4`, `PROP-5` | Release command, atomic visibility/audit boundary, scoped Participant reads, and availability-only notification outbox — architecture approved by [ADR-009](../../architecture/decisions/ADR-009-mvp-session-evaluation-review-contracts.md) and the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Separate authority, pre-release denial, exact participant, duplicate/conflicting Release, audit failure, lost response, projection/notification race tests | Release preview/confirmation/pending/success/error; participant pending/released/denied states | Gap |
| `REQ-REL-14`–`REQ-REL-16`, `AC-REL-13`–`AC-REL-15`, `PROP-6` | Correction lineage and current-visible-Result resolver — architecture approved in the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Replacement no-side-effect, correction race, new Release, prior lineage, lawful unavailability tests | Corrected Result, update notice, historical/unavailable states | Gap |
| `REQ-REV-27`–`REQ-REV-35`, `AC-REV-12`, `AC-REV-15`, `AC-REV-16`, `AC-REV-18`–`AC-REV-20`, `PROP-10` | Authorization adapters, audit/outbox, versioned lifecycle-policy resolver/enforcer, export hooks, and learning-disabled controls — architecture approved in [ADR-002](../../architecture/decisions/ADR-002-authorization-enforcement-and-delegation.md), [ADR-003](../../architecture/decisions/ADR-003-authorization-audit-persistence.md), and the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Full wrong-scope/identifier matrix, revocation, audit durability/redaction, export injection/isolation, configured duration/disposition, upper-bound narrowing, missing/widening policy, effective-time/migration, non-reuse and reconstruction tests | Permission denied, redacted, export, lifecycle status, unavailable and audit/provenance views | Gap |
| UX requirements, `AC-REL-10`–`AC-REL-12`, `PROP-9` | Review-side behavior is approved in the [Evidence, Evaluation, and Human Review interaction specification](../../ui-ux/evidence-evaluation-human-review.md); Result/Release interaction specification and frontend implementation TBD | Component accessibility and end-to-end keyboard/screen-reader tests TBD | Required Playwright accessibility snapshots and desktop/narrow screenshots across applicable states | Gap |
| Performance requirements, `AC-REV-17`, `PROP-8` | Scoped query, command, projection, and backpressure boundaries — architecture approved in the [Review/Release contract](../../architecture/review-result-release-contract.md); implementation TBD | Representative p95 reads/acknowledgments/visibility, saturation, outage, projection lag, retry-storm and recovery tests | Pending, reconciling, delayed and recovery messaging | Gap |
| Upstream/downstream end-to-end boundary | P0 #1–#7 integration — implementation TBD | Configure-to-release happy path plus isolation, integrity, failure, correction, and no-implicit-release integration tests | Complete reviewer and participant journey at desktop and narrow widths | Gap |
