...
- WCAG 2.2 AA is the approved requirements baseline under `PROP-9`; a downstream UI/UX specification must define the detailed interaction and visual contract.
...
- Approved UI/UX interaction specification for review queues, criterion/Evidence inspection, Human revision, decision, Result preview, Release confirmation, participant Result, conflict, correction, and accessibility states.
...
| `Q-9`, `PROP-9` | Use WCAG 2.2 AA as the requirements baseline across desktop and narrow layouts. | Provides a testable inclusive baseline while detailed interactions remain owned by a downstream UI/UX specification. |
...
- `PROP-9` — Use WCAG 2.2 AA as the review and Result requirements baseline; detailed interaction design remains subject to an approved UI/UX specification.
...
| UX requirements, `AC-REL-10`–`AC-REL-12`, `PROP-9` | Review/result interaction specification and frontend — UI/UX spec and implementation TBD | Component accessibility and end-to-end keyboard/screen-reader tests TBD | Required Playwright accessibility snapshots and desktop/narrow screenshots across applicable states | Gap |
