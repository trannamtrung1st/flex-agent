...
- Architecture decision for review-candidate/decision/Result records, lifecycle-policy ownership and enforcement, concurrency and idempotency, and the atomic Release/visibility/audit boundary before implementation.
...
| --- | --- | --- | --- | --- |
| `REQ-REV-1`–`REQ-REV-8`, `AC-REV-1`–`AC-REV-4`, `PROP-1` | Review-case/assignment state machine, trusted handoff, candidate selector, scoped queue — architecture and implementation TBD | Eligibility, no-Evaluation, exact-version, replacement, stale candidate, wrong-scope list/count, claim/reassign/revoke concurrency tests | Awaiting, ready, assigned, stale, unresolved, denied states | Gap |
| `REQ-REV-9`–`REQ-REV-18`, `AC-REV-5`–`AC-REV-9`, `PROP-2` | Authorized inspection adapters, revision schema/validator, content classification and rendering — implementation TBD | Source reauthorization, field allowlist/range, Evidence link, immutability, note leakage, unsafe-content, chain-of-thought exclusion tests | Evaluation/Evidence inspection, revision form, validation, focus return, internal/participant content separation | Gap |
| `REQ-REV-19`–`REQ-REV-26`, `AC-REV-6`, `AC-REV-10`–`AC-REV-15` | Decision state machine, expected-version/idempotency boundary, immutable lineage, ADR-003 audit — architecture and implementation TBD | Approve unchanged, revised approve, reject, escalate, stale permission, concurrent/duplicate/conflicting decision, audit failure tests | Decision confirmations, conflict/recovery, approved/rejected/escalated states | Gap |
| `REQ-REL-1`–`REQ-REL-5`, `AC-REL-1`, `AC-REL-2`, `PROP-3` | Versioned Result schema, participant projection, protected-content and rendering validators — implementation TBD | Allowlist/completeness/range/aggregation, internal-field leakage, locale, markup/link, integrity tests | Result preview, validation, unavailable and unsafe-content states | Gap |
| `REQ-REL-6`–`REQ-REL-13`, `AC-REL-3`–`AC-REL-12`, `PROP-4`, `PROP-5` | Release command, atomic visibility/audit boundary, scoped participant reads, notification outbox — ADR and implementation TBD | Separate authority, pre-release denial, exact participant, duplicate/conflicting Release, audit failure, lost response, projection/notification race tests | Release preview/confirmation/pending/success/error; participant pending/released/denied states | Gap |
| `REQ-REL-14`–`REQ-REL-16`, `AC-REL-13`–`AC-REL-15`, `PROP-6` | Correction lineage and current-visible-Result resolver — architecture and implementation TBD | Replacement no-side-effect, correction race, new Release, prior lineage, lawful unavailability tests | Corrected Result, update notice, historical/unavailable states | Gap |
| `REQ-REV-27`–`REQ-REV-35`, `AC-REV-12`, `AC-REV-15`, `AC-REV-16`, `AC-REV-18`–`AC-REV-20`, `PROP-10` | ADR-002 authorization adapters, ADR-003 audit/outbox, versioned lifecycle-policy resolver/enforcer, export hooks, learning-disabled controls — architecture and implementation TBD | Full wrong-scope/identifier matrix, revocation, audit durability/redaction, export injection/isolation, configured duration/disposition, upper-bound narrowing, missing/widening policy, effective-time/migration, non-reuse and reconstruction tests | Permission denied, redacted, export, lifecycle status, unavailable and audit/provenance views | Gap |
...
| Performance requirements, `AC-REV-17`, `PROP-8` | Scoped query, command, projection, and backpressure implementation — architecture TBD | Representative p95 reads/acknowledgments/visibility, saturation, outage, projection lag, retry-storm and recovery tests | Pending, reconciling, delayed and recovery messaging | Gap |
