...
| Other `.work/active/*` | Planned contract work only | Interaction Controller contract is planned; completed hosted-text-session, pagination, reset, harness-correction, and Attempt-start records are retired from `.work/active` |

## Active work

`.work/active/text-interaction-controller-contract.md` is `planned` and not
activated. Hosted-text-session (`12bfc65`, completion review PASS,
Implementation 33899790628) and other completed implementation records
(Participants cursor-pager, server-numbered pagination, reset,
harness-correction, Attempt-start) are retired from `.work/active`; recover
them from Git.
