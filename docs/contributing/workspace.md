...
| `GATE-STACK-POSTGRES` | Partial — Grate empty/repeat/changed-script plus Session runtime migrations `0005`–`0029`, human-authentication migrations `0030`–`0033` including Docker-backed upgrade execution, and scoped repository isolation/concurrency tests against PostgreSQL 18; backup/restore remain later |
...
| `GATE-STACK-PROVIDERS` | Partial — deterministic fake plus the legacy Direct OpenAI official-SDK adapter isolated in `FlexAgent.Sessions.OpenAi`, fake-transport contract tests, frozen per-Session profile/credential authority, and fail-closed Worker composition. ADR-008/ADR-010 now target `openai_compatible` / `sessions.openai_compatible.v1`; implementation and durable-identity migration have not started, so the legacy adapter remains default-off and is not qualification evidence for the target contract. The distinct OpenRouter synthetic-development adapter, fail-closed Worker composition, deterministic contracts, persistent live-request budget, and bounded live harness are implemented and independently reviewed. Pinned-route live evidence remains partial: Lightning streaming passed identity, cache, metadata, and usage checks, while Lightning, Gemma/Darkbloom, and Nano backup structured-control probes all failed Decision admission, so the profile is not qualified or enabled for real data. Interactive hosted chat, exact OpenAI-compatible profile qualification, Organization-hosted private-endpoint destination-policy evidence, and vLLM contract evidence also remain |
