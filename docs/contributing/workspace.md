...
| `GATE-STACK-PROVIDERS` | Partial — deterministic fake plus Direct OpenAI official-SDK adapter isolated in `FlexAgent.Sessions.OpenAi`, fake-transport contract tests, frozen per-Session profile/credential authority, and fail-closed Worker composition. The OpenRouter synthetic-development profile is approved, but its distinct adapter, fake-transport contracts, live synthetic evidence, and interactive local-chat evidence are not implemented. Exact Direct OpenAI profile qualification and vLLM contract evidence also remain |
