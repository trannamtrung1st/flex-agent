...
| `GATE-STACK-POSTGRES` | Partial — Grate empty/repeat/changed-script plus Session runtime migrations `0005`–`0027` and scoped repository isolation/concurrency tests against PostgreSQL 18; backup/restore and full-module isolation remain later |
...
| `GATE-STACK-PROVIDERS` | Partial — deterministic fake plus Direct OpenAI official-SDK adapter isolated in `FlexAgent.Sessions.OpenAi`, fake-transport contract tests, frozen per-Session profile/credential authority, and fail-closed Worker composition. Exact-profile live qualification and synthetic OpenRouter/vLLM contract evidence remain |
