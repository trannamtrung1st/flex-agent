...
| `GATE-STACK-POSTGRES` | Partial — Grate empty/repeat/changed-script plus Session runtime migrations `0005`–`0029`, human-authentication migrations `0030`–`0033`, Assessment migrations `0034`–`0042`, Enrollment assignment `0043`, shared admission `0044`–`0045`, and accommodations `0046`, including Docker-backed upgrade execution and scoped repository isolation/concurrency/idempotency/audit tests against PostgreSQL 18; backup/restore remain later |
