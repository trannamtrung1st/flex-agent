...
| `GATE-STACK-POSTGRES` | Partial — Grate empty/repeat/changed-script plus Session runtime migrations `0005`–`0029` and scoped repository isolation/concurrency tests against PostgreSQL 18; backup/restore and full-module isolation remain later |
...
| `GATE-STACK-SESSION` | Partial — synthetic SSE reconnect/replay and production HTTP SSE host wiring preserve Session sequence and scoped reauthorization; production opaque application-session rotation/revocation remains later |
...
| `GATE-STACK-HTTP`, `GATE-STACK-ARTIFACTS` | Deferred — OIDC/HTTP contract suite and SeaweedFS remain later sequenced artifacts |
